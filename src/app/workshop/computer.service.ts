...
import { disengage, echo, engage, loadNavData, plot } from "./computer.actions";
...
export class ComputerService{
    ...
    public InterpretDirectives(directives: IComputerDirective[]){
        ...
        directives.forEach(x => {
            switch(x.verb){
                case 'disengage':
                    this.store.dispatch(disengage({adverb: x.adverb, directObject: x.directObject, adjectivalPhrase: x.adjectivalPhrase}))
                    break;
                case 'engage':
                    this.store.dispatch(engage({adverb: x.adverb, directObject: x.directObject, adjectivalPhrase: x.adjectivalPhrase}))
                    break;
                case 'plot':
                    this.store.dispatch(plot({adverb: x.adverb, directObject: x.directObject, adjectivalPhrase: x.adjectivalPhrase}))
                    break;
                default:
                    break;
            }
